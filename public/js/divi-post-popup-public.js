(function( $ ) {
	...
	$(function () {
		...
		function initModal() {
			...
			if ($window.width() >= fromPHP.breakpoint) {
				...
				$("body").on("click", 'article.th-display-popup a', function (e) {
					// Define variables
					...
					// Load content from internal page
						...
						// when loading any other type of content
							...
							if (fromPHP.restMethod || $(this).hasClass("rest")) {
								$.get(
									fromPHP.siteUrl +
									"/wp-json/divi-post-popup/v1/any-post-type?slug=" +
									postSlug,
									function (response) {
	...
	$(document).ready(function() {
		...
		$('article.th-display-popup a').on( 'click', function(event) {
			
			// prevent default
			event.preventDefault();
			var post_title = $(this).text();
			var post_id = $(this).closest('article.th-display-popup').attr('id');
			...
			$.ajax({
				...
				success: function (data) {
					...
					$('#th-modal-heading').html(post_title);
					$('#th-modal-content').html(data);
					myModal.show();
				},
		...
		$(document).mouseup(function(e) {
			var popup = $("#th-myPopup");
			if (!popup.is(e.target) && popup.has(e.target).length === 0) {
				popup.hide();
			}
		});

		$("#th-popupCloseBtn").click(function() {
			$("#th-myPopup").hide();
		});
